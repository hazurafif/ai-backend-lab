...
async def create_skill(body: schemas.SkillIn, _: dict = Depends(get_current_user)):
...
@router.get("/skills/{name}", response_model=schemas.SkillOut)
...
@router.put("/skills/{name}", response_model=schemas.SkillOut)
async def update_skill(name: str, body: schemas.SkillIn, _: dict = Depends(get_current_user)):
    try:
...
@router.get("/tools", response_model=list[schemas.ToolServerOut])
async def list_tool_servers(_: dict = Depends(get_current_user)):
    return await resources.list_tool_servers(persistence.store)


@router.post("/tools", response_model=schemas.ToolServerOut, status_code=201)
async def create_tool_server(body: schemas.ToolServerIn, _: dict = Depends(get_current_user)):
...
@router.get("/tools/{name}", response_model=schemas.ToolServerOut)
...
@router.put("/tools/{name}", response_model=schemas.ToolServerOut)
async def update_tool_server(
    name: str, body: schemas.ToolServerIn, _: dict = Depends(get_current_user)
):
    try:
