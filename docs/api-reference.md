...
Detailed reference for the AI Backend service. The README links here for the
long-form docs: endpoint table, SSE contract, agent configs, MCP wiring,
knowledge-base tuning, HITL and sharing.
...
## Stored connections & runtime settings

Admin-managed connections (`base URL` + `api token`) live in the DB and drive
the agent's chat model and KB embeddings instead of
...
Runtime overrides via `GET|PUT /settings` — values are stored in the DB
(`app_settings` table) and take effect on the next agent run (backend +
graphs rebuild immediately):
...
The contract above is **normative** — `app/main.py`'s module docstring is the
source of truth and must stay in sync with `services/chat.py`.
...
- Query rewriting (opt-in, `KB_QUERY_REWRITE=true`): an LLM call rewrites
  ...
  query).
- Embeddings resolution order: saved `embeddings` connection →
  `EMBEDDINGS_MLX_URL` (local MLX, Qwen3-Embedding-0.6B — see README) →
  `OPENAI_API_KEY` (`EMBEDDINGS_MODEL`, default `text-embedding-3-small`) →
  deterministic local embedder (dev/tests).
